...
export default function App() {
  ...
  // تحديث الساعة الرقمية بشكل مستقل وثابت
  useEffect(() => {
    const timer = setInterval(() => setCurrentTime(new Date()), 1000);
    return () => clearInterval(timer);
  }, []);

  // المزامنة اللحظية الفورية الخالية من الحلقات التكرارية (تم تصفير مصفوفة التحكم)
  useEffect(() => {
    let isMounted = true;
    async function fetchQuantumData() {
      try {
        // حظر تخزين المتصفح وضمان المعالجة الآنية بأقل من ثانية
        ...
        if (isMounted && data) {
          ...
          // إدارة المؤقتات والـ Beep بشكل وظيفي منعزل كلياً عن الـ Re-render
    ...
    fetchQuantumData();
    // ضبط التحديث اللحظي الآمن والمستقر كل 3 ثوانٍ
...
const QuantumStaff = ({ employees, tickets }) => (
  <div className="w-full space-y-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
        {employees.map(emp => {
            const load = tickets.filter(t => t.engineer && String(t.engineer).includes(emp.name)).length;
            return ( 
