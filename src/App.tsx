...
export default function App() {
  ...
  // 1. تصفية "الساحة الحية" -> تعتمد فقط على الـ Status لاستبقاء السيارات غير المسلّمة نهائياً
  const liveYardTickets = useMemo(() => {
     let playBeep = false;
     const currentTimers = { ...readyTimers };

     const filtered = allRawCars.map((t, idx) => {
     ...
     }).filter(t => !t.status.includes("تسليم") && !t.status.includes("تم التسليم")); // تختفي فقط عند التسليم النهائي

     if (playBeep) playReadySound();
     return filtered;
  }, [allRawCars, readyTimers]);

  // حجب وإخفاء السيارات الجاهزة بعد انقضاء 4 دقائق من الساحة الحية
  ...
  // 2. تصفية المقبوضات الحية للوردية غير المغلقة (مرحل !== true) لكي تتصفر عند إغلاق الوردية
  const currentReceipts = useMemo(() => {
     ...
     }).filter(t => t.isArchived !== true && t.isArchived !== "TRUE" && t.isArchived !== "true");
  }, [allRawCars]);

  // 3. تصفية المصروفات غير المغلقة (مرحل !== true) لكي تتصفر عند إغلاق الوردية
  const currentExpenses = useMemo(() => {
     ...
     }).filter(e => e.isArchived !== true && e.isArchived !== "TRUE" && e.isArchived !== "true");
  }, [allRawExpenses]);

  // الحسابات المالية اللحظية للوردية الحالية
    ...
    <div className="min-h-screen w-full bg-[#030712] flex flex-col font-sans select-none overflow-hidden relative">
      <header className="w-full glass-panel border-b border-[#162235] px-6 py-4 flex flex-row justify-between items-center z-10">
        <div className="flex items-center gap-4">
          {/* كبسولة الشعار الدائرية الاحترافية المتوهجة والمطابقة لشعارك الفعلي */}
      ...
      {/* خط التوجيه اليساري الشامل للموقع بالكامل */}
  ...
  <div className="w-full space-y-6">
    <div className="glass-panel rounded-2xl p-6 shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
            ...
            <div key={t.id} className={`bg-[#050914] border ${glow} rounded-2xl p-5 flex flex-col justify-between transition-all duration-300 hover:scale-[1.02] group w-full shadow-xl ${isReadyBlink ? 'ready-blink' : ''}`}>
              <div>
                ...
                <div className="bg-[#090d16] p-3.5 rounded-xl border border-[#142033] mb-4">
                  <span className="text-[10px] text-slate-500 block">Task / العطل المطلوب:</span>
                  <div className="text-xs text-slate-200 leading-relaxed font-medium mt-1">{t.problem}</div>
                </div>
              </div>
        ...
        {tickets.length === 0 && <div className="text-slate-500 col-span-full py-10 text-center font-bold">الساحة المركزية خالية حالياً.</div>}
      </div>
    </div>
  );
};
