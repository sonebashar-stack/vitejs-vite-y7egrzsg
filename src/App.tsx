...
const IconUsers = () => <svg xmlns="http://www.w3.org/2000/xl" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>;
...
// حقن تأثير الوميض الخارجي بدون التعديل على ملفات التصميم
...
// دالة التنبيه الصوتي النقي (Web Audio API)
const playReadyBeep = () => {
  try {
    ...
    console.error("Audio protocol deferred.");
  }
};
...
export default function App() {
  ...
  // بيانات الفنيين الثابتة من كودك الأصلي
  ...
  // ميكانيكية مزامنة وقراءة البيانات السحابية الحية بدقة تامة وبدون حساسية للمسافات
  useEffect(() => {
    let isMounted = true;
    async function fetchQuantumData() {
      try {
        ...
        if (Array.isArray(data) && isMounted) {
          ...
          // استبعاد البيانات الفارغة والسيارات التي تم ترحيل ورديتها بالكامل
          ...
          const parsedTickets = liveRows.map((t, idx) => {
            ...
            // تتبع حركات السيارات الجاهزة لإطلاق الصوت والوميض
            ...
            return {
              ...
              customer: getCleanValue(t, ["اسم الزبون", "الزبون", "اسم العميل"]) || "-"
            };
          });

          if (playBeep) playReadyBeep();
          setReadyTimers(currentTimers);
          setTickets(parsedTickets.reverse()); // الأحدث يظهر أولاً باللوحة
          setError(null);
        }
      } catch (err) {
        console.error(err);
    ...
    fetchQuantumData();
    const loop = setInterval(fetchQuantumData, 10000); // مزامنة آمنة كل 10 ثواني
    return () => { isMounted = false; clearInterval(loop); };
  }, [readyTimers]);

  // تصفية وحجب السيارات الجاهزة بعد انقضاء 4 دقائق كاملة من الساحة
  ...
  // إحصائيات لوحة الـ Dashboard بناءً على الحركات الحية
  ...
  const financeStats = useMemo(() => {
    ...
    displayTickets.forEach(t => {
        ...
        if (t.payment.includes('كليك') || t.payment.includes('CliQ')) totalCliqIn += t.cost;
    });
    const netProfit = (totalCashIn + totalCliqIn + extIncome) - expTotal;
    return { totalCashIn, totalCliqIn, extIncome, expTotal, netProfit };
  }, [displayTickets]);

  // --- مكونات العرض (تصميمك الأصلي ومقاييسك الفخمة بالكامل) ---
  const ViewLiveYard = () => (
    <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {displayTickets.map(t => {
                        ...
                        if(t.status.includes('جاهز') || t.status.includes('تسليم')) { statusColor = "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 font-black"; isReadyBlink = true; }

                        return (
                            <div key={t.ticketId} className={`bg-slate-950/50 border rounded-2xl p-4 flex flex-col justify-between border-slate-800/80 hover:border-slate-700 transition ${glow} ${isReadyBlink ? 'ready-blink-card' : ''}`}>
                                ...
                                <div className="flex justify-between items-center border-t border-slate-800 pt-3">
                                    ...
                                    <span className="text-[10px] text-slate-500 font-bold">{t.customer.split(' ')[0]}</span>
                                </div>
                            </div>
                        )
                    })}
            ...
            {!isLoading && displayTickets.length === 0 && (
                <div className="text-center py-10 text-slate-500 text-sm">الساحة المركزية خالية من المركبات النشطة.</div>
            )}
        </div>
    </div>
  );
